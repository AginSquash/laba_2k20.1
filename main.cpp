...
#define DEBUG
#define Procedure_Macro

//#include "OOP/DT.h"
#include "Procedure/DT_Procedure.h"
...
int Procedure() {
    int year; int mon; int day; int hour; int min; int sec;
#ifdef DEBUG
    create(2020, 3, 9, 14, 05, 00);
#endif
#ifndef DEBUG
    cout << "Введите год, месяц, день, час, минуту и секунду" <<endl;
    cin >> year >> mon >> day >> hour >> min >> sec;
    create(year, mon, day, hour, min, sec);
#endif
    cout << "Что вы хотите сделать?\n 1 - добавить дату\n 2 - вычесть дату\n 3 - проверить дату\n 4 - добавить N лет/дней/месяец/etc\n 5 - выйти" << endl;
    bool cont = true;
    while (cont)
    {
        ...
        switch (chose) {
            case 1:
                cout << "Введите год, месяц, день, час, минуту и секунду" <<endl;
                cin >> year >> mon >> day >> hour >> min >> sec;
                addDateTime(year, mon, day, hour, min, sec);
                cout << getDateTime() << endl;
                break;
            case 2:
                cout << "Введите год, месяц, день, час, минуту и секунду" <<endl;
                cin >> year >> mon >> day >> hour >> min >> sec;
                cout <<  subtractDateTime(year, mon, day, hour, min, sec) << endl;
                break;
            case 3:
                cout << "Введите год, месяц, день, час, минуту и секунду" <<endl;
                cin >> year >> mon >> day >> hour >> min >> sec;
